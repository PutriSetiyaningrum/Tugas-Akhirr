@extends('layouts_user.main')
@section('container')
            <!-- slider -->
            <section class="slider home3">
                <div class="elip-header4 animate-rotate">
                    <img src="/../../assets/user/images/mark-page/basket.png" alt="img">
                </div>
                <div class="elip-header5 animate-up">
                    <img src="/../../assets/user/images/mark-page/mark-header-9.png" alt="img">
                </div>
                <div class="container-fuild">
                    <div class="swiper-container mainslider">                            
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="flat-slider">  <div class="overlay-slider style"></div>
                                <div class="sld-img"><img src="/../../assets/user/images/image-slider/slider01-home3.jpg" alt=""></div>
                                    <div class="content">
                                        <div class="heading">
                                            <h1>SELAMAT DATANG <br>
                                                SOBAT BASKET . . . <br>
                                            <p>Yuk cari tau tentang event basket yang sedang dilaksanakan 
                                            </p>
                                        </div>
                                        
                                    </div>
                                </div>                           
                            </div>          
                        </div>
                    </div>
                </div>               
            </section> 
@endsection